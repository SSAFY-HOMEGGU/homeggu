...
const ModelReadyModal = ({ isOpen, onClose, objUrl, onUploadComplete }) => {
  ...
  const handleUpload = async () => {
    ...
    //   formData.append('files', selectedFile); // 'files'라는 이름으로 파일 추가
  
    //   const response = await uploadGoodsImage(formData); // FormData 객체 전달
    ...
    // store에 3D 이미지 경로 업데이트
      set3DImages(response);

      // 전체 상품 정보로 업데이트 요청
      const updateData = {
        salesBoardDTO,
        goodsImagePaths,
        threeDimensionsGoodsImagePaths: response
      };
